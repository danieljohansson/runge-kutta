/**
 * Returns an array with n linearly spaced numbers between a and b
 */
...
/**
 * Returns an array with n logarithmically spaced numbers between a and b
 */
...
var numToStr = function (x, precision) {
    ...
    sign = (x < 0) ? '-' : '';
    x = Math.abs(x);
    exp = Math.floor(Math.log(x) / Math.LN10); // log base 10
    
    // 4 digits default precision
...
var matrixPrettyPrint = function (arr, maxRows, maxCols, sparse) {
    ...
    for (i = 0; i < m && i < maxRows; i++) {
        // [].slice.call to handle typed arrays
        rows[i] = [].slice.call(arr[i], 0, maxCols).join('\t');
        if (tooManyCols) {
